        ...
        <template v-for="group in leftGroups" :key="group.id">
            ...
            <ul v-if="group.items.length > 0">
                <li v-for="item in group.items" :key="`${group.id}:${item.route}`">
                    <mc-link :route="item.route" :params="item.params" :class="{'active': active(item)}">{{item.label}}</mc-link>
                </li>
            </ul>
...
<nav v-if="viewport=='mobile'" class="panel-nav" :entity="entity" :class="classes">
    ...
    <div class="panel-nav">
        <li><mc-link :entity='entity'><?= i::__('Meu Perfil') ?></mc-link></li>
        <li><mc-link route='auth/logout'><?= i::__('Sair') ?></mc-link></li>
    </div>
</nav>